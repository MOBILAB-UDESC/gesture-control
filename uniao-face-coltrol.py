...
def detect():
    global left_hand_center, right_hand_center, center_face, hands_detected, admin_in_the_house, frame, read, fps
    ...
    with mp_face_detection.FaceDetection(model_selection=1, min_detection_confidence=0.5) as face_detector, \
            mp_hands.Hands(static_image_mode=False, max_num_hands=2, min_detection_confidence=0.5) as hands_detector:

        while admin_in_the_house:
            if not read:
                continue
...
while True:
    ...
    for name in names:
        if name == 'deborah' and not admin_in_the_house:
            print('ACHEI A DEBORAH')
            admin_in_the_house = True
            camThread = threading.Thread(target=detect)
            camThread.start()
            break
        elif name != 'deborah':
            print('não é a Deborah')
            admin_in_the_house = False
